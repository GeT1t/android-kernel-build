	...
	for (iter = cvector_begin(g_vHwBpInfo); iter != cvector_end(g_vHwBpInfo); iter = cvector_next(g_vHwBpInfo, iter)) {
		...
		if (hwbpInfo->sample_hbp == sample_hbp) {
			for (child = cvector_begin(hwbpInfo->vHit); child != cvector_end(hwbpInfo->vHit); child = cvector_next(hwbpInfo->vHit, child)) {
				hitInfo = (struct HIT_INFO *)child;
				memset(&userHitInfo, 0, sizeof(userHitInfo));
...
static ssize_t hwBreakpointProc_write(struct file* filp, const char __user* buf, size_t size, loff_t *ppos) {
	citerator iter;
	struct HW_BREAKPOINT_INFO * hwbpInfo = NULL;
	for (iter = cvector_begin(g_vHwBpInfo); iter != cvector_end(g_vHwBpInfo); iter = cvector_next(g_vHwBpInfo, iter)) {
		hwbpInfo = (struct HW_BREAKPOINT_INFO *)iter;
...
static void sample_hbp_handler(struct perf_event *bp,
	...
	struct HIT_INFO * ptrHitInfo = NULL;
	struct HIT_INFO hitInfo;
	struct HW_BREAKPOINT_INFO hwbpInfo = { 0 };
	struct HW_BREAKPOINT_INFO * ptrHwbpInfo = NULL;
	...
	for (iter = cvector_begin(g_vHwBpInfo); iter != cvector_end(g_vHwBpInfo); iter = cvector_next(g_vHwBpInfo, iter)) {
		ptrHwbpInfo = (struct HW_BREAKPOINT_INFO *)iter;
		if (ptrHwbpInfo->sample_hbp == bp) {
			exist_hbp = 1;
			exist_hit = 0;

			//printk_debug(KERN_INFO "hwbpInfo->sample_hbp==bp\n");
			for (child = cvector_begin(ptrHwbpInfo->vHit); child != cvector_end(ptrHwbpInfo->vHit); child = cvector_next(ptrHwbpInfo->vHit, child)) {
				ptrHitInfo = (struct HIT_INFO *)child;
				if (ptrHitInfo->hit_addr == now_hit_addr) {
					exist_hit = 1;
					ptrHitInfo->hit_count++;
					break;
				}
			}
			if (exist_hit == 0) {
				...
				cvector_pushback(ptrHwbpInfo->vHit, &hitInfo);
			}
			break;
		}
	}
	...
	if (exist_hbp == 0) {
		...
		memset(&hwbpInfo, 0, sizeof(hwbpInfo));
		memset(&hitInfo, 0, sizeof(hitInfo));
...
static long hwBreakpointProc_ioctl_openprocess(unsigned long arg) {
	char buf[8];
	int64_t pid = 0;
...
static long hwBreakpointProc_ioctl_closehandle(unsigned long arg) {
	char buf[8];
	struct pid * proc_pid_struct = NULL;
...
static long hwBreakpointProc_ioctl_set_hwbp_hit_conditions(unsigned long arg) {
...
static long hwBreakpointProc_ioctl_add_process_hwbp(unsigned long arg) {
	//获取进程句柄
	char buf[28];
	struct pid * proc_pid_struct = NULL;
...
static long hwBreakpointProc_ioctl_del_process_hwbp(unsigned long arg) {
	char buf[8];
	struct perf_event * sample_hbp = NULL;
	citerator iter;
...
static long hwBreakpointProc_ioctl_get_hwbp_hit_addr_count(unsigned long arg) {
	char buf[8];
	struct perf_event * sample_hbp = NULL;
	size_t count = 0;
	citerator iter;
...
static long hwBreakpointProc_ioctl(
	...
	case IOCTL_OPEN_PROCESS: //打开进程
	{
		...
		*/
		...
		return hwBreakpointProc_ioctl_openprocess(arg);
		break;
	}
	case IOCTL_CLOSE_HANDLE: //关闭进程
	{
		...
		*/
		...
		//获取进程句柄
		return hwBreakpointProc_ioctl_closehandle(arg);
		break;
	}
	...
	case IOCTL_SET_HWBP_HIT_CONDITIONS: //设置硬件断点命中记录条件
	{
		...
		*/
		...
		//获取硬件断点句柄
		return hwBreakpointProc_ioctl_set_hwbp_hit_conditions(arg);
		break;
	}
	case IOCTL_ADD_PROCESS_HWBP: //设置进程硬件断点
	{
		...
		*/
		...
		return hwBreakpointProc_ioctl_add_process_hwbp(arg);
	...
	case IOCTL_DEL_PROCESS_HWBP: //删除进程硬件断点
	{
		...
		*/
		...
		return hwBreakpointProc_ioctl_del_process_hwbp(arg);
		break;
	}
	case IOCTL_GET_HWBP_HIT_ADDR_COUNT: //获取硬件断点命中地址数量
	{
		...
		*/
		return hwBreakpointProc_ioctl_get_hwbp_hit_addr_count(arg);
		break;
	}
	default:
		return -EINVAL;
...
static void __exit hwBreakpointProc_dev_exit(void) {
	...
	struct HW_BREAKPOINT_INFO * hwbpInfo = NULL;
	for (iter = cvector_begin(g_vHwBpHandle); iter != cvector_end(g_vHwBpHandle); iter = cvector_next(g_vHwBpHandle, iter)) {
		sample_hbp = (struct perf_event *)*((size_t*)iter);
		unregister_hw_breakpoint(sample_hbp);
	}
